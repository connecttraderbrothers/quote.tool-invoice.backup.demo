...
function updateQuoteTable() {
    ...
    for (var i = 0; i < items.length; i++) {
        var item = items[i];
        html += '<tr><td>' + item.category + '</td><td>' + item.description + '</td><td class="text-center">' + item.quantity + '</td><td class="text-right">Â£' + item.unitPrice.toFixed(2) + '</td><td class="text-right" style="font-weight:600">Â£' + item.lineTotal.toFixed(2) + '</td><td class="text-center"><div style="display:flex;gap:5px;justify-content:center;flex-wrap:wrap"><button class="btn-action btn-edit" onclick="editItem(' + i + ')">Edit</button><button class="btn-action btn-move" onclick="moveItem(' + i + ',\'up\')"' + (i === 0 ? ' disabled' : '') + '>Up</button><button class="btn-action btn-move" onclick="moveItem(' + i + ',\'down\')"' + (i === items.length - 1 ? ' disabled' : '') + '>Dn</button><button class="btn-action btn-reposition" onclick="repositionItem(' + i + ')">#</button><button class="btn-action btn-delete" onclick="removeItem(' + i + ')">Del</button></div></td></tr>';
    }
