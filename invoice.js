...
function updateInvoiceTable() {
    ...
    for (var i = 0; i < invoiceItems.length; i++) {
        ...
        html += '<tr><td>' + item.category + '</td><td>' + item.description + '</td><td class="text-center">' + item.quantity + '</td><td class="text-right">Â£' + item.unitPrice.toFixed(2) + '</td><td class="text-right" style="font-weight:600">Â£' + item.lineTotal.toFixed(2) + '</td><td class="text-center"><div style="display:flex;gap:5px;justify-content:center;flex-wrap:wrap"><button class="btn-action btn-edit" onclick="editInvoiceItem(' + i + ')">Edit</button><button class="btn-action btn-move" onclick="moveInvoiceItem(' + i + ',\'up\')"' + (i === 0 ? ' disabled' : '') + '>Up</button><button class="btn-action btn-move" onclick="moveInvoiceItem(' + i + ',\'down\')"' + (i === invoiceItems.length - 1 ? ' disabled' : '') + '>Dn</button><button class="btn-action btn-reposition" onclick="repositionInvoiceItem(' + i + ')">#</button><button class="btn-action btn-delete" onclick="removeInvoiceItem(' + i + ')">Del</button></div></td></tr>';
    }
